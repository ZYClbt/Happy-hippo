...
def draw_bbox(frame, classes, body_box, body_color, txt_color=(255, 255, 255), subset='train'):
    ...
    for cls_id, box in zip(classes, body_box):
...
def process_label(label_list, height, width):
    ...
    for label in label_list:
        cls_id, centerx, centery, bboxw, bboxh = label.strip('\n').split(' ')
        cls_id, centerx, centery, bboxw, bboxh = int(cls_id), float(centerx)*width, float(centery)*height, float(bboxw)*width, float(bboxh)*height
        # print(cls_id, centerx, centery, bboxw, bboxh)
        minx, miny, maxx, maxy = centerx - bboxw/2, centery - bboxh/2, centerx + bboxw/2, centery + bboxh/2
        bbox = np.array([minx, miny, maxx, maxy])
        ret['cls'].append(cls_id)
        ret['bbox'].append(bbox)
    return ret

def main():
    os.makedirs(osp.join(data_dir, 'vis_labels_detect'), exist_ok=True)
    os.makedirs(osp.join(data_dir, 'vis_labels_detect', 'train'), exist_ok=True)
    os.makedirs(osp.join(data_dir, 'vis_labels_detect', 'val'), exist_ok=True)
    label_path_list = read_files(osp.join(data_dir, 'labels_detect'))
    print('total file num of labels-detection: ', len(label_path_list))
    
    global counts, train_counts, val_counts
    body_color = (46, 74, 246)

    for label_path in tqdm(label_path_list):
        image_path = label_path.replace('labels_detect', 'images').replace('.txt', '.jpg').replace('train/', '').replace('val/', '')
        ...
        cv2.imwrite(label_path.replace('labels_detect', 'vis_labels_detect').replace('.txt', '.jpg'), image)

    print(f'total annot num of bboxes: {counts} | train {train_counts}, val {val_counts}')
